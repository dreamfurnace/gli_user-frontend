    ...
    <div v-else class="relative">
      ...
      >
        <div :class="[
          ...
          isAuthenticated ? 'bg-green-300 animate-pulse' : 'bg-yellow-300 animate-bounce'
        ]"></div>
        <span class="font-medium">
          {{ isAuthenticated ? shortAddress : '인증 필요' }}
        </span>
        <ChevronDownIcon class="w-4 h-4 transition-transform" :class="{ 'rotate-180': showDropdown }" />
      </button>
        ...
        >
          ...
          <div class="p-4 border-b border-white/10">
            <div class="flex items-center gap-3 mb-3">
              <div class="w-10 h-10 rounded-lg flex items-center justify-center" style="background: var(--gli-gradient-primary)">
                <PhantomIcon class="w-6 h-6 text-white" />
              </div>
              <div>
                <h3 class="gli-emoji-enhanced font-semibold text-white">팬텀 월렛 💎</h3>
          ...
          <div class="p-4 space-y-2">
            ...
            >
              <div v-if="isAuthenticating" class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
              <span v-else class="text-lg">🚀</span>
              <span>{{ isAuthenticating ? 'GLI 인증 중...' : 'GLI 플랫폼 인증하기' }}</span>
            </button>

            <!-- 인증 완료 시 표시 -->
            <div v-if="isAuthenticated" class="gli-glass-panel px-3 py-2 border-green-400/30">
            ...
            >
              <div v-if="isAirdropping" class="w-4 h-4 border-2 border-white/60 border-t-transparent rounded-full animate-spin"></div>
              <GiftIcon v-else class="w-4 h-4" />
              <span>{{ isAirdropping ? '에어드랍 중...' : '테스트 SOL 받기 🎁' }}</span>
            </button>
            ...
            >
              ...
              <span>{{ isAuthenticated ? 'GLI 로그아웃 🚪' : '지갑 연결 해제 🚪' }}</span>
            </button>
          </div>
        </div>
      </Transition>
