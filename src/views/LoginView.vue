  ...
  <div class="login-page-bg">
    <div class="login-container">
      ...
      <div class="signup-section">
        ...
        <button class="signup-button" @click="handleSignup">회원가입 하기</button>
      </div>
    </div>
    <div v-if="isDevelopment" class="dummy-login-floating">
      <div class="dummy-title">더미 계정 원클릭 로그인</div>
      <button class="dummy-btn" @click="dummyLogin(dummyUsers[0])" :disabled="authStore.isLoading">
        관리자 ({{ dummyUsers[0].email }})
      </button>
      <button class="dummy-btn" @click="dummyLogin(dummyUsers[1])" :disabled="authStore.isLoading">
        토큰 매니저 ({{ dummyUsers[1].email }})
      </button>
      <button class="dummy-btn" @click="dummyLogin(dummyUsers[2])" :disabled="authStore.isLoading">
...
// 더미 계정 정보
const dummyUsers = [
  { email: 'admin@gli.com', password: 'admin123!', role: 'admin', label: '관리자' },
  { email: 'token@gli.com', password: 'token123!', role: 'token_manager', label: '토큰 매니저' },
  { email: 'user@gli.com', password: 'user123!', role: 'user', label: '일반 사용자' },
]
...
const pendingUser = ref<{ email: string; password: string; role: string } | null>(null)
...
const handleLogin = async (event: Event) => {
  ...
  try {
    ...
    if (error.response?.data?.sessions) {
      pendingUser.value = { email: email.value, password: password.value, role: '' }
...
const dummyLogin = async (user: { email: string; password: string; role: string }) => {
  authStore.clearError()
...
const handleForceLogout = async () => {
  ...
  try {
    // 강제 로그아웃 API 호출
    const response = await fetch('http://localhost:8000/api/auth/force-logout/', {
      method: 'POST',
      headers: {
