...
export const authAPI = {
	...
	forceLogout: (data: {
		device_type: string;
		username: string;
	}): Promise<AxiosResponse> => apiClient.post("/auth/force-logout/", data),
...
export const profileAPI = {
	...
	// 특정 사용자 프로필 조회
	getProfileById: (userId: string): Promise<AxiosResponse> =>
	...
	uploadAvatar: (file: File): Promise<AxiosResponse> => {
		...
		console.log("[DEBUG] 요청 URL:", `${API_BASE_URL}/user/profile/avatar/`);
	...
	changePassword: (passwordData: {
		...
		apiClient.post("/user/profile/change-password/", passwordData),

	// 휴대폰 인증
	verifyPhone: (phoneData: {
		phone: string;
	...
	updateSubscription: (subscriptionData: {
		...
		apiClient.post("/user/profile/update-subscription/", subscriptionData),

	// 회원 탈퇴
	deleteAccount: (): Promise<AxiosResponse> => apiClient.delete("/user/profile/"),
};

// 사용자 관련 API
export const userAPI = {
	// 사용자 목록 조회
	getUsers: (params?: ApiParams): Promise<AxiosResponse> =>
...
export const gliContentAPI = {
	// Business Content API
	getBusinessContent: (params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get("/v1/business-content/", { params }),
	
	getBusinessContentBySection: (section: string): Promise<AxiosResponse> =>
		apiClient.get(`/v1/business-content/by_section/?section=${section}`),

	// Shopping Mall API
	getShoppingCategories: (params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get("/v1/shopping/categories/", { params }),

	getShoppingProducts: (params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get("/v1/shopping/products/", { params }),

	getShoppingProduct: (productId: string): Promise<AxiosResponse> =>
		apiClient.get(`/v1/shopping/products/${productId}/`),

	getFeaturedProducts: (): Promise<AxiosResponse> =>
		apiClient.get("/v1/shopping/products/featured/"),

	getProductsByCategory: (categoryId: string, params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get(`/v1/shopping/products/by_category/?category_id=${categoryId}`, { params }),

	// Shopping Orders API
	getShoppingOrders: (params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get("/v1/shopping/orders/", { params }),

	createShoppingOrder: (orderData: any): Promise<AxiosResponse> =>
		apiClient.post("/v1/shopping/orders/", orderData),

	getShoppingOrder: (orderId: string): Promise<AxiosResponse> =>
		apiClient.get(`/v1/shopping/orders/${orderId}/`),

	cancelShoppingOrder: (orderId: string): Promise<AxiosResponse> =>
		apiClient.post(`/v1/shopping/orders/${orderId}/cancel/`),

	confirmOrderPayment: (orderId: string, paymentData: { payment_tx_hash: string }): Promise<AxiosResponse> =>
		apiClient.post(`/v1/shopping/orders/${orderId}/confirm_payment/`, paymentData),

	// RWA Investment API
	getRWACategories: (params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get("/v1/rwa/categories/", { params }),

	getRWAAssets: (params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get("/v1/rwa/assets/", { params }),

	getRWAAsset: (assetId: string): Promise<AxiosResponse> =>
		apiClient.get(`/v1/rwa/assets/${assetId}/`),

	getFeaturedRWAAssets: (): Promise<AxiosResponse> =>
		apiClient.get("/v1/rwa/assets/featured/"),

	getRWAAssetsByCategory: (categoryId: string, params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get(`/v1/rwa/assets/by_category/?category_id=${categoryId}`, { params }),

	getRWAAssetsByRiskLevel: (riskLevel: string): Promise<AxiosResponse> =>
		apiClient.get(`/v1/rwa/assets/by_risk_level/?risk_level=${riskLevel}`),

	investInRWAAsset: (assetId: string, investmentData: { 
		amount_gleb: number; 
		amount_usd_at_time?: number; 
	}): Promise<AxiosResponse> =>
		apiClient.post(`/v1/rwa/assets/${assetId}/invest/`, investmentData),

	// Investment Portfolio API
	getInvestments: (params?: ApiParams): Promise<AxiosResponse> =>
		apiClient.get("/v1/investments/", { params }),

	getInvestment: (investmentId: string): Promise<AxiosResponse> =>
		apiClient.get(`/v1/investments/${investmentId}/`),

	getInvestmentStats: (): Promise<AxiosResponse> =>
		apiClient.get("/v1/investments/stats/"),

	getInvestmentPortfolio: (): Promise<AxiosResponse> =>
		apiClient.get("/v1/investments/portfolio/"),

	// Dashboard API
	getDashboardOverview: (): Promise<AxiosResponse> =>
		apiClient.get("/v1/dashboard/overview/"),
};

// Convenient aliases for backward compatibility and easier access
export const {
	// Business Content
	getBusinessContent,
	getBusinessContentBySection,
	...
	getRWAAssets,
	getRWAAsset,
	getFeaturedRWAAssets,
	getRWAAssetsByCategory,
	getRWAAssetsByRiskLevel,
	investInRWAAsset,
